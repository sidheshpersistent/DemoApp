...
import { PopupFailedOfficeMailVerification } from "../..";
...
describe("PopupFailedOfficeMailVerification Test Module", () => {
  test("Should match snapshot", async () => {
    const { toJSON } = await waitFor(() => render(<PopupFailedOfficeMailVerification />));
    expect(toJSON()).toMatchSnapshot();
  });
  test("Should fire the function passed as prop when onpress",async () => {

    const { getByTestId } = render(<PopupFailedOfficeMailVerification {...props}/>);
