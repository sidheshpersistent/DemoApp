...
describe("styled component Test Module", () => {
  test("Should match snapshot", async () => {
    const { toJSON } = await waitFor(() => render(<ApplicationBoxContainer {...props} />));
    expect(toJSON()).toMatchSnapshot();
  });
  test("test color flag prop", async () => {
    const { toJSON } = await waitFor(() => render(<ApplicationBoxContainer {...props1} />));
    expect(toJSON()).toMatchSnapshot();
  });
});
